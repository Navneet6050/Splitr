  ...
  async ({ step }) => {
    ...
    const users = await step.run("Fetch users with expenses", async () => {
      const activeUsers = await db.user.findMany({
        where: {
          email: { not: null },
          NOT: { email: { endsWith: "@import.local" } }, // Skip temporary imports
          OR: [
            { expensesPaid: { some: { date: { gte: startOfLastMonth } } } },
            { splits: { some: { expense: { date: { gte: startOfLastMonth } } } } },
          ],
        },
      });
    ...
    for (const user of users) {
      ...
      const expenses = await step.run(`Expenses · ${user.id}`, async () => {
